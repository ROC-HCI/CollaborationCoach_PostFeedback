...
#done
for file in Data/$session_id*.webm;
	do ffmpeg -i "$file" -codec:v copy -codec:a aac Data/testFixed5_$(basename "${file/.webm}").mp4
done
echo "Finish - .webm Header Correction"

echo "Start - .wav Conversion"
for file in Data/testFixed5_$session_id*.mp4;
	do ffmpeg -i "$file" -ar 11025 Data/$(basename "${file/.webm}").wav
done
echo "Finish - .wav Conversion"

echo "Start - Praat"
for file in Data/testFixed5_$session_id*.wav;
...
argpath=""
for i in Data/testFixed5_$session_id*.wav.TextGrid;
