...
#done
for file in Data/$session_id*.webm;
	do ffmpeg -i "$file" -codec:v copy -codec:a copy Data/testFixed3_$(basename "${file/.webm}").webm
done
echo "Finish - .webm Header Correction"

echo "Start - .wav Conversion"
for file in Data/testFixed3_$session_id*.webm;
	do ffmpeg -i "$file" -ar 11025 Data/$(basename "${file/.webm}").wav
done
echo "Finish - .wav Conversion"

echo "Start - Praat"
for file in Data/testFixed3_$session_id*.wav;
...
argpath=""
for i in Data/testFixed3_$session_id*.wav.TextGrid;
