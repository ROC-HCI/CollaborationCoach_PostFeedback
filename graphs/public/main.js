$(function() {
    ...
    var s = Snap('#guest0');
    Snap.load('./svg/Turn_taking_bllue.svg', function(fragment){
...
function drawArrows(count){
  ...
  var s = Snap('#guest-1');
  if(count===1){
    Snap.load('./svg/straight-arrow.svg',function(f){
      // var g = f.select('g:nth-of-type(1)').clear();
      s.append(f);
    });
  }
  else if(count===2){
    Snap.load('./svg/3_ppl_arrows.svg',function(f){
      s.append(f).attr({
        'y': 60
      });
    });
  }else if(count===3){
    Snap.load('./svg/4_ppl_arrows.svg',function(f){
      s.append(f).attr({
        'y': 60
      });
    });
    ...
    //4 guests
    Snap.load('./svg/5_ppl_arrows.svg',function(f){
      s.append(f).attr({
        'y': 60
      });
    });
...
function loado(){
  $('svg[id^="guest"]:not("#guest0")').each(function(){
    ...
    var s = Snap('#guest'+id);
    // s.clear();
    Snap.load('./svg/Turn_taking_yellow.svg',function(fragment){
      s.append(fragment);
    });
  });
